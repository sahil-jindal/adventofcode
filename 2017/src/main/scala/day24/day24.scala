...
def strongestBridge(input: Set[Component])(using ordering: Ordering[Pair]): Int = {
    def fold(pinIn: Int, components: Set[Component]): Pair = {
        ...
        for (component <- components.toList) {
            ...
            if (pinOut != -1) {
                ...
                strongest = ordering.max(updated, strongest)
            }
        }
...
def evaluatorOne(input: Set[Component]): Int = strongestBridge(input)(using partOneOrdering)
def evaluatorTwo(input: Set[Component]): Int = strongestBridge(input)(using partTwoOrdering)
