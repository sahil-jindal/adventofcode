...
def solve(numbersList: List[List[Int]], extrapolate: List[Int] => Int): Int = {
    return numbersList.map(extrapolate).sum
}
...
def evaluatorOne(input: List[List[Int]]): Int = solve(input, extrapolateRight)
def evaluatorTwo(input: List[List[Int]]): Int = solve(input, extrapolateLeft)
